...
export function usePendingSushi(farm) {
  ...
  const result = useSingleCallResult(args ? contract : null, 'pendingBonus', args)?.result
...
export function useChefPositions(contract?: Contract | null, rewarder?: Contract | null, chainId = undefined) {
  ...
  const pendingSushi = useSingleContractMultipleData(args ? contract : null, 'pendingBonus', args)
